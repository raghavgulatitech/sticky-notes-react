...
class Note extends Component {
    ...
    saveNote = function (e) {
        ...
        const model = {
            UserId: user.id,
            NoteHeading: this.new_title.value,
            NoteText: this.new_text.value
        }
        axios.post('https://localhost:44399/user/savenote', model)
            .then((response) => {
                ...
                <div className="modal-dialog m-auto py-0 px-3 col-12 col-md-8">
                    <div className="modal-content note">
                        <form className="d-contents" onSubmit={this.saveNote}>
                            ...
                            <div className="modal-footer">
                                {/* <button onClick={this.deleteNote} type="button"
                                    className="btn btn-outline-danger">
                                    <span className="react-icon" role="img"
    ...
    renderDefaultMode() {
        return (
            <div onClick={this.enterReadingMode} id={this.props.id} style={{ paddingTop: '10px' }}
